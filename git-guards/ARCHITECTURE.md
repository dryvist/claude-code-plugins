...
every matching tool invocation.
...
flowchart TD
    ...
    class PG,TG,MBG hook
    class PG_BLOCK,PG_CONFIRM,MBG_BLOCK block
    class PG_ALLOW,MBG_ALLOW allow
```
...
`permissionDecision: deny` on stdout while still exiting 0.
...
| Effect | Hard block or confirmation prompt | Soft guidance and conventions |
