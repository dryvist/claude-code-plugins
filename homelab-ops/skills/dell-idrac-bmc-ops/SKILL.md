...
infrastructure. Covers: resolving the true latest release (ISO + upgrade,
never ISO alone), selecting disks by serial not device name, the
build-time-baked answer file trap, per-generation virtual-media transport,
setting a one-time boot override, and detaching media after the final
dist-upgrade. Full sequence:
[references/virtual-media-install.md](references/virtual-media-install.md).
