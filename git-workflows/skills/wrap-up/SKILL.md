...
- **Path A** (complete): Every TaskList task is complete (or empty), AND every plan-file checklist item is checked or complete (or no plan file exists).
- **Path B** (incomplete): Any TaskList task or plan checklist item remains incomplete.
...
Use the "Recommended Prompt for Next Session", "Recommended GitHub Issues", and "Session Issues Log" sections generated by `/session-status` in Step 0. Format the output according to the standard layout provided by the `/session-status` report.

If no follow-up items are found in the `/session-status` report, state that explicitly — do not fabricate work.
